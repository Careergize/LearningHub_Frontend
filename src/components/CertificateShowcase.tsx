...
export default function CertificateShowcase() {
  ...
  const [specialization, setSpecialization] = React.useState("Artificial Intelligence & Machine Learning");
  const [credentialId, setCredentialId] = React.useState("CG-AI-902847-X");
  const [isCopied, setIsCopied] = React.useState(false);
  const [isVerifying, setIsVerifying] = React.useState(false);
  const [verifiedState, setVerifiedState] = React.useState(true);

  // Generate a random credential hash ID based on specialization
  React.useEffect(() => {
    const prefix = specialization.includes("Artificial") ? "CG-AI" : specialization.includes("MERN") ? "CG-MERN" : "CG-DATA";
  ...
  return (
    <section id="credentials" className="py-20 max-w-7xl mx-auto px-6">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
        
        {/* Left column configuration parameters (5/12 width) */}
        <div className="lg:col-span-5 space-y-8">
          <div>
            <div className="text-xs font-bold text-brand-primary uppercase tracking-widest mb-2">DIGITAL SECURE BADGES</div>
            <h2 className="font-sans font-extrabold text-3xl md:text-5xl text-brand-dark tracking-tight leading-tight">
              Verified Digital <br />
              <span className="text-brand-primary">Credentials.</span>
            </h2>
            <p className="font-sans text-sm md:text-base text-brand-dark/60 mt-3 leading-relaxed">
              Every course completion logs a tamper-proof digital certificate onto our public ledger, fully shareable directly onto your LinkedIn profile. Try editing the credentials live!
            </p>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-brand-dark/5 shadow-md space-y-4">
            ...
            <div>
              ...
              >
                ...
                <option value="MERN Full-Stack System Architecture">MERN Full-Stack System Architecture</option>
                <option value="Data Science & Quantitative Pipelines">Data Science & Quantitative Pipelines</option>
              </select>
            </div>
            ...
            <div className="pt-2 flex gap-3">
              ...
              >
                {isVerifying ? "Querying Ledgers..." : "Verify Registry"}
              </button>
              <button 
        ...
        {/* Right column live interactive certificate renderer (7/12 width) */}
          ...
          <div className="relative w-full max-w-xl">
            ...
            <div className="bg-white rounded-3xl p-8 md:p-10 shadow-2xl border-8 border-brand-surface relative overflow-hidden flex flex-col justify-between aspect-[1.414/1] w-full bg-[radial-gradient(#e2e8f0_1px,transparent_1px)] [background-size:16px_16px]">
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-brand-primary text-xl font-bold">rocket_launch</span>
              ...
              <div className="text-center my-6 space-y-3.5">
                ...
                <p className="text-[10px] text-brand-dark/40 italic">This verified document is proudly presented to</p>
                ...
                <p className="text-[10px] text-brand-dark/50 max-w-md mx-auto leading-relaxed">
                  For successfully meeting all course criteria and technical logic challenges of the career transformation syllabus in <span className="font-bold text-brand-dark">{specialization}</span>.
                </p>
              </div>

              {/* Certificate Footer with serial code, registry status, signature */}
              <div className="flex justify-between items-end border-t border-brand-dark/5 pt-4">
                <div className="text-left space-y-1">
                  <div className="text-[9px] font-bold text-brand-dark/40 uppercase">Credential ID</div>
                  <div className="font-mono text-[10px] text-brand-dark font-semibold">{credentialId}</div>
                  <div className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
                    <span className="text-[9px] text-emerald-600 font-bold uppercase">Registry Status: ACTIVE</span>
                  </div>
                </div>
                ...
                <div className="text-right space-y-1">
                  ...
                  <div className="text-[9px] font-bold text-brand-dark/40 uppercase">Dean of Systems Engineering</div>
                </div>
              </div>
            ...
            <AnimatePresence>
              {isCopied && (
                ...
                >
                  <span className="material-symbols-outlined text-emerald-400 text-4xl font-bold">check_circle</span>
                  <h4 className="font-sans font-extrabold text-lg">Shared Successfully!</h4>
                  <p className="font-sans text-xs text-white/70 max-w-xs text-center leading-relaxed">
                    Credential link copied! You can now paste this URL as a verified badge directly onto your LinkedIn profile or portfolio page.
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
